...
;(function($) {
    ...
    $(function() {
        ...
        // Load masonry
        $( '.exopite-multifilter-container.masonry' ).each(function(index, el) {

            var dataJSON = $( el ).data('ajax');

            var wrapper_id = $( el ).attr( 'id' );

            // Initialize waterfall-kudago
            $( '#' + wrapper_id + ' .exopite-multifilter-items' ).waterfall({
                colMinWidth: dataJSON.col_min_width || 340,
                // defaultContainerWidth: 1020,
                autoresize: true
            });
            ...
            if ( ( '#' + wrapper_id + ' .exopite-multifilter-items:has(img)' ) ) {
                ...
                // Refresh after image loaded
                // If image load too fast, will not align images right, also
                // if lazyloading, images load too late
            ...
            // After AJAX get ne elements, realign
            var event_name = ( dataJSON.pagination == 'pagination' ) ? 'paginated' : 'appended';

            $( this ).on( 'success-items-' + event_name + '.exopiteMultifilter', function( event, base, articles ) {

                triggerRecalculateMasonry( wrapper_id );
            ...
            // Debounce refresh
            var masonryRefreshDebounce = Exopite.debounce(function() {
